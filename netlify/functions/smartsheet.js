...
const HEADERS = {
  ...
  // Master Schedule — read by column POSITION (1st/2nd/3rd), not title.
  // We don't have the sheet's exact header text confirmed, only that the
  // first three columns are Project, Production Start Date, Production Due Date.
  MasterSchedule: ['Project','ProductionStartDate','ProductionDueDate'],
};
...
async function handleSync(tab) {
  ...
  // Master Schedule has no confirmed column titles — map by position instead.
  if (tab === 'MasterSchedule') {
    ...
    const rows = (sheet.rows || []).map(row => {
      ...
      return [0, 1, 2].map(i => {
        ...
        const v = col ? cellByCol[col.id] : undefined;
  ...
  // Existing tabs — mapped by column title, unchanged.
  const colByTitle = {};
